...
if(Debug){
  int Gx = values.GyX - 2.6;
  int Gy = values.GyY - 0.6;
  int Gz = values.GyZ + 0.3;
  float gradox= map (values.AcX,-0.99,0.99,-90.0,90.0);
  float gradoy= map (values.AcY,-0.99,0.99,-90.0,90.0);
  float gradoz= map (values.AcZ,-0.99,0.99,-90.0,90.0);
  
Serial.print(" GyX = "); Serial.print(Gx);
Serial.print(" °/s| GyY = "); Serial.print(Gy);
Serial.print(" °/s| GyZ = "); Serial.print(Gz);
...
Serial.print(" g| Gx = "); Serial.print(gradox);
Serial.print(" g| Gy = "); Serial.print(gradoy);
Serial.print(" g| Gz = "); Serial.println(gradoz);
}
